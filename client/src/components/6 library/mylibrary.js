...
function DraggableNavigationMenu() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      setUser(prevUser => {
        ...
        console.log("User data mylib:", newUser.email);
  ...
  const handlePdfClick = async (uniqueId, accessType) => {
    try {
      ...
      const response = await axios.post('http://localhost:5000/userLibrary/libx', { uniqueId, accessType, person }, {
        responseType: 'blob',
      });
  ...
  return (
    <div className='mylib-body'>
      <nav
        className={isNavOpen ? "open mylib-nav" : "mylib-nav"}
            ...
            <div className="mylib-row">
              <div className="mylib-row1 mylib-rows" >
                ...
                <ul>
                  {pdfDataRecent.map((pdfr, index) => (
                    <li key={index}>
                      <div className="mylib-pdf" ref={mylibPdfRef}
                        onClick={() => handlePdfClick(pdfr.file_id, "recent")}
                      >
                        <div className='ss'>
                          <div className='options-icon'
                            onClick={(event) => handleOptionsIconClick(index, false, false, true, event)}>
                            <i className="fa-solid fa-ellipsis-vertical" ></i>
                          </div>
                          <div className={`options ${recentOptionsListActiveIndex === index ? 'active-options-list' : ''}`}>
                            <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(true, false, false); openModaldeleteres(); setPdfFileid(pdfr.file_id); }}
                              onMouseEnter={() => handleToolTip(index, true, false, false, true)}
                              onMouseLeave={() => handleToolTipclose()}><i className="fa-solid fa-trash"></i></p>
                            <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(true, false, false); handlechangepdftype(event, pdfr.file_id, "private") }}
                              onMouseEnter={() => handleToolTip(index, false, true, false, true)}
                              onMouseLeave={() => handleToolTipclose()}><i className="fa-solid fa-lock-open"></i></p>
                            <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(true, false, false); }}
                              onMouseEnter={() => handleToolTip(index, false, false, true, true)}
                              onMouseLeave={() => handleToolTipclose()}><i className="fa-regular fa-bookmark"></i></p>
                          </div>
                        </div>
              ...
              <div id="myModal" className="modal" style={{ display: modalOpendeleteres ? 'block' : 'none' }} onClick={handleOutsideClick}>
                <div className="modal-content">
                  ...
                  <div className='text-btns'>
                    <button className='p' onClick={(e) => { Optionclose(true); deletePdf(e, true, "recent"); closeModal(); }}>YES</button>
                    <button className='p' onClick={(e) => { Optionclose(true); closeModal() }}>NO</button>
                  </div>
                </div>
              </div>
                        ...
                        if (!searchPrivateQuery || matchesSearch) {
                          return (
                            <li key={index} className="row">
                              <div className="mylib-pdf" ref={mylibPdfRef}
                                onClick={() => handlePdfClick(pdf.file_id, "private")}
                              >
                                <div className='ss'>
                                  <div className='options-icon'
                                    onClick={(event) => handleOptionsIconClick(index, true, false, false, event)}>
                                    <i className="fa-solid fa-ellipsis-vertical" ></i>
                                  </div>
                                  <div className={`options ${privateOptionsListActiveIndex === index ? 'active-options-list' : ''}`}>
                                    <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(true, false, false); openModaldeletepriv(); setPdfFileid(pdf.file_id); }}
                                      onMouseEnter={() => handleToolTip(index, true, false, false, true)}
                                      onMouseLeave={() => handleToolTipclose()}><i className="fa-solid fa-trash"></i></p>
                                    <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(true, false, false); handlechangepdftype(event, pdf.file_id, "private") }}
                                      onMouseEnter={() => handleToolTip(index, false, true, false, true)}
                                      onMouseLeave={() => handleToolTipclose()}><i className="fa-solid fa-lock-open"></i></p>
                                    <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(true, false, false); }}
                                      onMouseEnter={() => handleToolTip(index, false, false, true, true)}
                                ...
                                <div className="mylib-matter">
                                  <p>{pdf.year}</p>
                                  <p>{pdf.semester}</p>
                                  <p>{pdf.branch},{pdf.subject_name}</p>
                                  <p>{pdf.unit_no},{`ref{${pdf.refer}}`}</p>
                                </div>
                              </div>
                            </li>
                          );
              ...
              <div id="myModal" className="modal" style={{ display: modalOpendeletepriv ? 'block' : 'none' }} onClick={handleOutsideClick}>
                <div className="modal-content">
                  ...
                  <div className='text1'>Are You sure you want to delete..!</div>
                  <div className='text-btns'>
                    <button className='p' onClick={(e) => { Optionclose(true); deletePdf(e, true, "private"); closeModal(); }}>YES</button>
                    <button className='p' onClick={(e) => { Optionclose(true); closeModal() }}>NO</button>
                  </div>
                </div>
              </div>
                      ...
                      {publicBranchGroups[branch].map((pdf, index) => {
                        ...
                        if (!searchPublicQuery || matchesSearch) {
                          return (
                            <li key={index} className="row">
                              <div className="mylib-pdf" ref={mylibPdfRef}
                                onClick={() => handlePdfClick(pdf.file_id, "public")}
                              >
                                <div className='ss'>
                                  <div className='options-icon'
                                    onClick={(event) => handleOptionsIconClick(index, false, true, false, event)}>
                                    <i className="fa-solid fa-ellipsis-vertical" ></i>
                                  </div>
                                  <div className={`options ${publicOptionsListActiveIndex === index ? 'active-options-list' : ''}`}>
                                    <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(false, true, false); openModaldeletepub(); setPdfFileid(pdf.file_id); }}
                                      onMouseEnter={() => handleToolTip(index, true, false, false, false)}
                                      onMouseLeave={() => handleToolTipclose()}><i className="fa-solid fa-trash"></i></p>
                                    <p onClick={(event) => { event.stopPropagation(); handleOptionItemClick(false, true, false); handlechangepdftype(event, pdf.file_id, "public") }}
              ...
              <div id="myModal" className="modal" style={{ display: modalOpendeletepub ? 'block' : 'none' }} onClick={handleOutsideClick}>
                <div className="modal-content">
                  ...
                  <div className='text1'>Are You sure you want to delete..!</div>
                  <div className='text-btns'>
                    <button className='p' onClick={(e) => { Optionclose(true); deletePdf(e, true, "public"); closeModal(); }}>YES</button>
                    <button className='p' onClick={(e) => { Optionclose(true); closeModal(); }}>NO</button>
                  </div>
                </div>
              </div>
