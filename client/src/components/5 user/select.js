        ...
        <div ref={dropdownRef} className={`user-change-custom-select ${isOpen ? 'open' : ''}`}>
            ...
            {isOpen && (
                <div className="user-change-options">
                    {options.map((option) => (
                        <div className="user-change-option" key={option} onClick={() => handleOptionClick(option)}>
                            {option}
                        </div>
                    ))}
                </div>
...
const Select = ({ updateCourses }) => {
    ...
    const handleButtonClick = () => {
        const updatedCourses = generate(selectedYear, selectedSemester, selectedBranch);
        updateCourses({
            updatedCourses,
            selectedYear,
            selectedSemester,
