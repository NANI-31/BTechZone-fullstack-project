...
function Signup() {
    ...
    const handleIconClick = (inputKey, iClass) => {
        setInputTypes((prevInputTypes) => ({
            ...prevInputTypes, [inputKey]: prevInputTypes[inputKey] === 'text' ? 'password' : 'text'
        }));

        setIconClass((prevIconClass) => ({
            ...prevIconClass, [iClass]: prevIconClass[iClass] === 'fa-solid fa-eye-slash' ? 'fa-solid fa-eye' : 'fa-solid fa-eye-slash'
        }));
    };
    const ValidateEmail = () => {
    ...
    const validatePassword = () => {
        ...
        if (password === confirmpassword) {
            if (passwordRegex.test(password)) {
                ...
                // alert("The password contains atleast 8 characters, one lowercase, one uppercase, one number, one special symbol");
                openModalp();
            }
        }
        else {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            console.log(selectedYear, selectedSemester, selectedBranch);
            ...
            const response = await axios.post('http://localhost:5000/signups/signup', { name, email, password, phoneno, selectedYear, selectedSemester, selectedBranch })
        ...
        <div className='signup-body'>
            ...
            <div id="signup-center">
                ...
                <div className="signup-form-box signup-login">
                    <div id="sign-up">
                        <h2 className="signup-h2"><span className="signup-multiple-text">Sign Up</span></h2>
                        <form name="signup" method="post" className='signup-form' onSubmit={handleSubmit}>

                            {/* <!----------------------------------user name    onsubmit="return checkpass();"------------------------------------> */}
                            <div className="signup-input-box b1">
                                <input type="text" name="username" id="username" autoComplete="off"
                                    onChange={(e) => setName(e.target.value)} />
                                <label>Username</label><i className="fa-solid fa-user"></i>
                            </div>
                            ...
                            <div className="signup-input-box b2">
                                ...
                                <label>E-mail</label><i className="fa-solid fa-envelope" autoComplete="off"></i>
                            </div>
                            ...
                            <div className="signup-input-box b3">
                                <input type="tel" name="phoneno" maxLength="10" id="phoneno" autoComplete="off"
                                    onChange={(e) => setPhoneno(e.target.value)} />
                                <label>Phone No</label><i className="fa-solid fa-phone"></i>
                            </div>
                            ...
                            <div className="signup-input-box b4">
                                ...
                                <label>Birth Date</label><i className="fa-regular fa-calendar"></i>
                            </div>

                            <div className="signup-input-box bbbb">
                                {/* {Object.keys(selects).map((key) => (
                                    <div key={key} className="user-change-custom-select">
                                        <div
                                            className="user-change-selected-option"
                                            onClick={() => toggleDropdown(key)}
                                        >
                                            <span>{selectedValues[key].selectedValue}</span>
                                            <i className={`fas ${selectedValues[key].isOpen ? 'fa-angle-up' : 'fa-angle-down'}`}></i>
                                        </div>
                                        {selectedValues[key].isOpen && (
                                            <div className="user-change-options">
                                                {selects[key].options.map((option) => (
                                                    <div
                                                        key={option}
                                                        className="user-change-option"
                            ...
                            <div className="signup-input-box b5">
                                <input type={inputTypes.input1} required id="password" name="password1" autoComplete="off"
                                    onChange={(e) => setPassword(e.target.value)} />
                                <label>password</label>

                                <i className={iconClass.iname1}
                                    onClick={() => handleIconClick('input1', 'iname1')}
                            ...
                            <div className="signup-input-box b6">
                                <input type={inputTypes.input2} id="cpassword" name="password2" autoComplete="off"
                                    onChange={(e) => setConfirmpassword(e.target.value)}
                                />

                                <label className=".q">confirm password</label>

                                <i className={iconClass.iname2}
                                    onClick={() => handleIconClick('input2', 'iname2')}
                                    id="ceyeicon" style={{ cursor: "pointer" }}></i>
                            ...
                            <div className="signup-input-box signup-ib1">
                                ...
                                <div className="signup-logreg-link">
                                    ...
                                    <p><Link to="/" className="register-link">Back to Home</Link></p>
                                </div>
                            </div>
                        </form>
                    </div>
                ...
                <div>
                    <div id="myModal" className="modal" style={{ display: modalOpene ? 'block' : 'none' }} onClick={handleOutsideClick}>
                        <div className="modal-content">
                            ...
                            <div className='text-btns'>
                                <button className='p' onClick={(e) => closeModal()}>OK</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="myModal" className="modal" style={{ display: modalOpenp ? 'block' : 'none' }} onClick={handleOutsideClick}>
                        <div className="modal-content">
                            ...
                            <div className='text1'>The password contains atleast 8 characters, one lowercase, one uppercase, one number, one special symbol</div>
                            <div className='text-btns'>
                                <button className='p' onClick={(e) => closeModal()}>OK</button>
                            </div>
                        </div>
